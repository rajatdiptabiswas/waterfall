...
def str_to_ec_point(ansi_str, ec_curve):
    ...
    x, y = str_to_int(ansi_str[: len(ansi_str) // 2]), str_to_int(
        ansi_str[len(ansi_str) // 2 :]
    )
...
class TLSConnection:
    ...
    def driveKeys(self):
        ...
        server_keypair = ec.Keypair(
            ec_curve, pub=str_to_ec_point(self.serverpub, ec_curve)
        )
        ...
        target_len = 128
        blockkey = self.prf.get_bytes(
            pshare,
            "key expansion",
            self.serverrandom + self.clientrandom,
    ...
    def decrypt(self, cipherdata):
        ...
        try:
            ...
            self.cipher = Cipher(
                algorithms.AES(self.server_write_key),
                self.mode,
                backend=backend,
            )  # AES.new(self.server_write_key,AES.MODE_CBC,self.server_write_IV) #
    ...
    def addDATA(self, data):
        self.datacarry += data
        # log.debug("TLSConnection - data=\n{}".format(data))
        # log.debug("TLSConnection - datacarry=\n{}".format(self.datacarry))
        ...
        while flag:
            ...
            log.warning("TLSConnection - GET COMMAND {}".format(cmd))

            size = struct.unpack(">I", self.datacarry[1:5])[0]
            if size + self.headersize <= len(self.datacarry):
                log.info("TLSConnection - cmd={} size={}".format(cmd, size))
    ...
    def addHTTPpacket(self, pkt):
        # log.debug("TLSConnection packet=\n{}".format(pkt))
        if not "/~milad" in pkt:
            return
    ...
    def getnewpayload(self, size, seq):
        ...
        log.debug("replacedpackets[seq={}]={}".format(seq, ret))
    ...
    def processTLSpacket(self, pkt):
        ...
        if scapy_ssl_tls.ssl_tls.TLSClientHello in mtls:
            ...
            # print [ord(i) for i in self.clientrandom]
            # log.debug(
            #     "`[ord(i) for i in self.clientrandom]` ".format(
            #         [ord(i) for i in self.clientrandom]
            #     )
            # )
        ...
        if scapy_ssl_tls.ssl_tls.TLSServerKeyExchange in mtls:
            ...
            sec_params = scapy_ssl_tls.ssl_tls_crypto.TLSSecurityParameters.from_pre_master_secret(
                self.prf,
                scapy_ssl_tls.ssl_tls.TLSCipherSuite.ECDHE_RSA_WITH_AES_128_GCM_SHA256,
                mk,
                self.clientrandom,
                self.serverrandom,
            )
    ...
    def addTLSPacket(self, pkt):
        ...
        while flag:
            try:
                plen = (
                    TLS(self.carry).records[0].length
                )  # [scapy_ssl_tls.ssl_tls.TLSRecord].length
                # TLS(self.carry).show2()
            except:
                # TLS(self.carry).show2()
                # log.debug(
