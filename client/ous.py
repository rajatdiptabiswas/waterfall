...
class ProxyUpstreamProtocol(protocol.Protocol):
    ...
    def connectionMade(self):
        ...
        """
        `self.factory.proxy` = `ProxyServerProtocol`
        """
        self.connected = True
    ...
    """
    `send_upstream` writes to `ProxyUpstreamProtocol` `transport`
    `send_downstream` writes to `ProxyServerProtocol` `transport`
    """
...
class ProxyServerProtocol(basic.LineReceiver):
    ...
    def lineReceived(self, line):
        ...
        `match.group(0)`: The entire matched string 'CONNECT www.example.com:443 HTTP/1.1'.
        `match.group(1)`: The captured hostname 'www.example.com'.
        `match.group(2)`: The captured port number '443'.
        """
    ...
    def end_of_headers(self):
        ...
        if self.ous.is_overt_address(self.address[0]):
            ...
            """
            `start_proxy()` www.example.com:443
            """
...
class OvertRequest(http.Request):
    ...
    def build_http_request(self, headers=None):
        ...
        """
        request_lines = [
            "{} {} {}".format(self.method, self.uri, self.clientproto)
        ]
        ...
        for header in _headers:
            request_lines.append(
                "{}: {}".format(header.title(), _headers[header])
            )
    ...
    class CacheRequestProtocol(protocol.Protocol):
        ...
        def done(self):
            ...
            self.factory.response_received(
                self.factory.request_id,
                self.factory.request.cache_key,
                response,
            )
            self.transport.loseConnection()
...
class OvertUserSimulator(object):
    ...
    def start_proxy(self):
        """
        Starts the proxy server by creating a `ProxyServerProtocol` instance and listening on the `proxy_port` 7070.
        """
    ...
    def start_proxy_handler(self):
        """
        Starts the proxy handler server by creating an `OvertHTTPChannel` instance and listening on the `proxy_handler_port` 6060. It uses an SSL context to enable secure communication.
        """

        """
        The `type` function in Python is a built-in function that can be used to dynamically create new classes by specifying the class name, base classes, and attributes/methods in a dictionary. It provides a powerful way to generate classes at runtime.
        ...
        - `bases` is a tuple of base classes from which the new class inherits. If there is only one base class, it can be specified directly without using a tuple.
        - `dict` is a dictionary containing the attributes and methods of the class.
        """
        httpchannel = type(
            "OvertHTTPChannel",
